...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    getBandwidthDataByDateWithSessionToken(bandwidthRequest)
      .then((response) => {
        const processedData = processBandwidthData(response);
    ...
    const bandwidthRequestForMax : BandwidthRequest = {
      session_token: getAuthenticationBody(),
      from: startDate,
      to: endDate,
  ...
  return (
    <div>
      ...
      ) : (<Error className="dashboard-error" errorMessage={errorMessage} />)}
    </div>
  );
};
